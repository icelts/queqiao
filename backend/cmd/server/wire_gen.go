...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	subscriptionPurchasePriceRepository := repository.NewSubscriptionPurchasePriceRepository(db)
	...
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, subscriptionPurchasePriceRepository, billingCacheService, client, configConfig)
	...
	adminService := service.NewAdminService(userRepository, groupRepository, accountRepository, soraAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, subscriptionPurchasePriceRepository, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory)
	...
	referralService := service.NewReferralService(client, settingService, subscriptionService, billingCacheService, apiKeyAuthCacheInvalidator)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, adminReferralHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler)
	...
	subscriptionHandler := handler.NewSubscriptionHandler(subscriptionService, referralService, xunhuPayService)
	rechargeHandler := handler.NewRechargeHandler(referralService, settingService, xunhuPayService)
	referralHandler := handler.NewReferralHandler(referralService)
	totpHandler := handler.NewTotpHandler(totpService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, rechargeHandler, referralHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, idempotencyCoordinator, idempotencyCleanupService)
